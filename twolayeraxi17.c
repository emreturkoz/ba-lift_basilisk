...
#define H0ABS 4.0280E-6
#define R0ABS 17.2240E-6
...
event init (t=0){
  ...
  refine ( x < 0.7  && level < MAXLEVEL + 2);
  refine ( y < 0.4  && level < MAXLEVEL + 2);
...
event output (t += 0.1*TAU; t<=2.0*TAU){
  static int nf= 0;
  char name[100];
  sprintf(name,"blister_%.2f.dat",t/TAU);
  ...
  char name2[100];
  sprintf(name2,"pressure/pressure_%.2f.ppm",t/TAU);
  FILE *fp2 = fopen(name2,"w");
  output_ppm(p,fp2, box = {{0.0,0.0},{4.0,1.0}}, n=1024);

  scalar velocity[];
  foreach()
    velocity[] = f23[]*sqrt(u.x[]*u.x[] + u.y[]*u.y[] + u.z[]*u.z[]);
  boundary ({velocity});

  char name3[100];
  sprintf(name3,"velocity/velocity_%.2f.ppm",t/TAU);
  FILE *fp3 = fopen(name3,"w");
  output_ppm(velocity,fp3, box = {{0.0,0.0},{4.0,1.0}}, n=1024);


  char name5[100];
  sprintf(name5,"f23_%0.2f.dat",t/TAU);
...
event jet_output (t += 2*TAU; t<=2000.0*TAU){
  char name[100];
  ...
  // evaluating the fluid layer - the difference btw. f23 and f12
  scalar fluid_layer[];
  foreach()
    fluid_layer[] = f23[]-f12[];
  boundary ({fluid_layer});
	

  char name2[100];
  sprintf(name2,"f23/f23_%04d.ppm",(int)(t/TAU));
  FILE *fp2 = fopen(name2,"w");
  output_ppm(fluid_layer,fp2, box = {{0.0,0.0},{4.0,1.0}}, n=1024);  


  // plotting the velocity as well
  scalar velocity[];
  foreach()
    velocity[] = f23[]*sqrt(u.x[]*u.x[] + u.y[]*u.y[] + u.z[]*u.z[]);
  boundary ({velocity});
  ...
  char name3[100];
  sprintf(name3,"velocity/velocity_%04d.ppm",(int)(t/TAU));
  FILE *fp3 = fopen(name3,"w");
  output_ppm(velocity,fp3, box = {{0.0,0.0},{4.0,1.0}}, n=1024);


  // prof. deike asked for the vorticity. 
  scalar omega[];
  vorticity(u,omega);
  char name4[100];
  sprintf(name4,"vorticity/vorticity_%04d.ppm",(int)(t/TAU));
  FILE *fp4 = fopen(name4,"w");
  output_ppm(omega,fp4, box = {{0.0,0.0},{4.0,1.0}}, min = -10, max = 10, n=1024);
