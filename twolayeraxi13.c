...
#define CALCMU(f12,f23) ( (MUA*(1-f23)*(1-f12)) + (MUL*f23*(1-f12)) + (MUS*f12)   )
...
event init (t=0){
  ...
  //refine ( x < 1.0  && level < MAXLEVEL + 1);
  //refine ( x < 1.0  && level < MAXLEVEL + 1);
  //refine ( y < 0.2  && level < MAXLEVEL + 1);
...
event gfsview (i += 1; t <= 100*TAU) {
  static FILE * fp = popen ("gfsview2D f23pressure.gfv", "w");
  scalar omega[];
  vorticity (u, omega);  
  ...
  scalar velocity[];
  foreach()
    velocity[] = f23[]*(1-f12[])*sqrt(u.x[]*u.x[] + u.y[]*u.y[]);
  output_gfs (fp, t = t);
...
event images (t += 0.1*TAU; t<=5.0*TAU){
  ...
  output_ppm(f23, fp2, 512, min=0, max=1, linear=true);
  ...
  static FILE * fp3 = fopen("velo.ppm","w");
  output_ppm(velocity, fp3, min=0, max=100, linear=true, n=1024);

  static FILE * fp4 = fopen("pressure.ppm", "w");
  output_ppm(p, fp4, n=512);
