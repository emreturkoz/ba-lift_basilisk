...
#define H0ABS 6.34E-6
#define R0ABS 17.665E-6
...
event moving_blister (i++) {
  ...
  coord vc = { VELSCALE/2 , 0.};
  coord tc = {(t<=2*TAU) ? 1 : 0,0.};
  foreach(){
    foreach_dimension(){
    u.y[] = tc.y*(f12[]*vc.y) + (1. - f12[])*u.y[]; //  +  ((1-tc.x)*(1-f12[])*u.x[]);
    //    u.y[] = 0.0; 	
    }
  }
  boundary ((scalar *){u});
...
event gfsview (i += 10; t <= 50.0) {
  static FILE * fp = popen ("gfsview2D velcontourvec.gfv", "w");
  scalar omega[];
  vorticity (u, omega);  
  output_gfs (fp, t = t);
}
